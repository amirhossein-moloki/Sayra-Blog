# گزارش وضعیت سیستم صفحه‌ساز (Page Builder) پروژه Playnest
...
سیستم صفحه‌ساز سورا (Playnest) بر پایه یک معماری **Registry-based** طراحی شده است. به این صورت که هر بخش از صفحه (Section) دارای یک تعریف مشخص در لایه کد، یک شمای اعتبارسنجی (Zod Schema) و یک رندرکننده (Renderer) اختصاصی است.
