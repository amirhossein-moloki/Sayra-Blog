# Playnest Deployment & Infrastructure Architecture Model

This document defines the production-ready infrastructure and deployment architecture for the Playnest platform.
...
### Namespaces
*   `playnest-prod`: Production workloads.
*   `playnest-monitoring`: Prometheus/Grafana.
*   `playnest-infra`: Ingress, External Secrets.
...
package "Public Cloud (AWS)" {
    ...
    package "VPC - Private Subnets" {
        node "EKS Cluster" as EKS {
            artifact "Playnest API Pods" as API
