# راهنمای نصب و اجرای محلی (بدون داکر) - Playnest

این راهنما برای توسعه‌دهندگانی است که مایلند پروژه Playnest را به صورت مستقیم بر روی سیستم‌عامل خود (Windows, macOS, Linux) و بدون استفاده از Docker اجرا کنند.
...
۲. **ایجاد دیتابیس:** وارد کنسول PostgreSQL شوید و یک دیتابیس جدید بسازید:
   ```sql
   CREATE DATABASE playnest_db;
   CREATE USER playnest_user WITH PASSWORD 'your_password';
   GRANT ALL PRIVILEGES ON DATABASE playnest_db TO playnest_user;
   ```
   *نکته: در نسخه‌های جدیدتر PostgreSQL، ممکن است نیاز باشد دسترسی Schema را نیز بدهید:*
   ```sql
   \c playnest_db
   GRANT ALL ON SCHEMA public TO playnest_user;
   ```
...
# git clone https://github.com/your-repo/playnest.git
# cd playnest
...
```env
DATABASE_URL="postgresql://playnest_user:your_password@localhost:5432/playnest_db"
