...
import { MenuLocation, UserRole, NavigationVisibilityRule, SessionActorType } from '@prisma/client';
...
describe('Phase 3 - Structural CMS Integration Tests (Mocked)', () => {
  ...
  describe('Pages Hierarchy', () => {
    it('should create a hierarchical page structure and compute fullPath', async () => {
      ...
      jest.mocked(pagesRepository.findPageByFullPath).mockResolvedValue(null);
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      jest.mocked(prisma.page.findUnique).mockResolvedValue(parent as any);
      jest.mocked(pagesRepository.createPage).mockImplementation((data, fullPath, depth) => Promise.resolve({
        ...data,
        id: 'child-1',
        fullPath,
        depth
    ...
    it('should prevent circular references', async () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      jest.mocked(pagesRepository.findPageById).mockResolvedValue({ id: 'page-1', fullPath: 'page-1', slug: 'page-1', parentId: null, depth: 0 } as any);
    ...
    it('should prevent moving a page under its descendant', async () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      jest.mocked(pagesRepository.findPageById).mockResolvedValue({ id: 'parent-1', fullPath: 'parent', slug: 'parent' } as any);
      jest.mocked(pagesRepository.findChildren).mockImplementation((id: string) => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        if (id === 'parent-1') return Promise.resolve([{ id: 'child-1' }] as any);
  ...
  describe('Navigation & Visibility', () => {
    it('should build a menu tree and apply visibility rules', async () => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      jest.mocked(navigationRepository.findMenuByLocation).mockResolvedValue({
        id: 'menu-1',
        items: mockItems
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
      } as any);
