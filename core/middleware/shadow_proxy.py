...
class ShadowProxyMiddleware:
    def __init__(self, get_response):
        ...
        self.playnest_url = getattr(settings, 'PLAYNEST_URL', 'http://localhost:3000/api/v1')
    ...
    def __call__(self, request):
        ...
        is_cms_request = request.path.startswith('/api/') and any(
            module in request.path for module in ['posts', 'categories', 'tags', 'comments', 'reactions', 'pages', 'navigation']
        )
        ...
        threading.Thread(
            target=self.shadow_request,
            args=(request, response, django_latency)
        ).start()
