...
def shadow_dashboard(request):
    ...
    avg_django_latency = (
        ShadowLog.objects.aggregate(Avg("django_latency_ms"))["django_latency_ms__avg"]
        or 0
    )
    avg_playnest_latency = (
        ShadowLog.objects.aggregate(Avg("playnest_latency_ms"))[
            "playnest_latency_ms__avg"
        ]
        or 0
    )

    severity_breakdown = ShadowLog.objects.values("mismatch_severity").annotate(
        count=Count("id")
    )
